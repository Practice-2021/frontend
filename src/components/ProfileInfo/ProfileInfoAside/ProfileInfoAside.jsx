...
const ProfileInfoAside = () => {
    ...
    return(
        <div className="flex bg-seashell py-2 px-3 md:py-5 md:px-6">
            ...
            <div>
                <p className="text-lg">Публикации: {teacher?.publications.length}</p>
                <p className="text-lg">Заявки: {teacher?.applications}</p>
                <p className="text-lg">Гранты: {teacher?.grants.length}</p>
                <p className="text-lg">Проекты: {teacher?.projects.length}</p>
            </div>
        </div>
    )
}
